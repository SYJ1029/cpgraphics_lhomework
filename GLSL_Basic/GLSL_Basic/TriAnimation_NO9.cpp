...
GLvoid Keyboard(unsigned char key, int x, int y) {
	...
	randnum[0] = (float)((float)rand() / RAND_MAX) * 10 + 10;

	
	float tokenx = 0, tokeny = 0, tokenz = 0;
	switch (key) {
	case '1':
		tokenx = 0.01, tokeny = 0.01, tokenz = 0;
		for (int i = 0; i < 4; i++) {
			randnum[i] = (float)((float)rand() / RAND_MAX) * 30 + 10;
			if (diagram[i].maked) {
				diagram[i].SetMoveType(MOVE_STRAIGHT);
				diagram[i].SetDelta(tokenx, tokeny, tokenz);
		...
		break;
	case '2':
		tokenx = 0.01, tokeny = 0, tokenz = 0;
		for (int i = 0; i < 4; i++) {
			randnum[i] = (float)((float)rand() / RAND_MAX) * 30 + 10;
			if (diagram[i].maked) {
				diagram[i].SetMoveType(MOVE_ZIGZAG);
				diagram[i].SetDelta(tokenx, tokeny, tokenz);
				diagram[i].SetMax(1, 5, 0, 1);
	...
	case '3':
		tokenx = 0.01, tokeny = -0.01, tokenz = 0;
		for (int i = 0; i < 4; i++) {
			randnum[i] = (float)((float)rand() / RAND_MAX) * 50;
			if (diagram[i].maked) {
				diagram[i].SetMoveType(MOVE_RECTSPIRAL);
				diagram[i].SetDelta(tokenx, tokeny, tokenz);
				diagram[i].SetMax(tokenx, tokeny, tokenz, 1);
	...
	case '4':
		for (int i = 0; i < 4; i++) {
			...
			if (diagram[i].maked) {
				diagram[i].SetMoveType(MOVE_CIRCLESPIRAL);
				if(i % 2 == 0)
					diagram[i].Settheta(1);
				else
					diagram[i].Settheta(-1);
				diagram[i].SetSpincenter();
